package com.force.http.rest
{
	...
	public class RESTConnection
	{
		...
		public function setSession(session:String, server:String):void {
			trace("INSTANCE IS::::::"+server);
			if(server.indexOf("-api") > 0) {server = server.replace("-api","");} //modify from LoginResult
			if(server.indexOf("/services") > 0) {server = server.split("/services")[0];}
		...
		public function tokenExpired():Boolean {
			if(oauthConnection == null) { return false; } //We're not maintaining an OAuth connecion
